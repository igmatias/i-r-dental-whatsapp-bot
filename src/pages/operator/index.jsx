...
const DEFAULT_SECRET = typeof window !== 'undefined' ? (new URLSearchParams(window.location.search).get('secret') || 'irdental2025') : 'irdental2025'
...
export default function OperatorConsole(){
  ...
  useEffect(() => {
    ...
    async function load(){
      try{
        ...
        if(!activeWa && data.chats?.[0]?.wa) setActiveWa(data.chats[0].wa)
      }catch{}
    }
    load()
  ...
  useEffect(() => {
    ...
    async function load(){
      try{
        const res = await fetch(`${API_BASE}/api/wsp/webhook?secret=${encodeURIComponent(secret)}&wa=${encodeURIComponent(activeWa)}&limit=300`)
        if(!res.ok) throw new Error('fetch messages failed')
        const data = await res.json()
        if(!alive) return
        
        const list = Array.isArray(data.messages) ? data.messages : []
        // Build stable keys even if id is missing/duplicated
        const keyed = list.map((m, i) => ({ ...m, _key: m.id || `${String(m.ts||0)}-${i}` }))
        // Merge with previous state to avoid flicker
        setMessages(prev => {
          const map = new Map()
      ...
      <main style={{ display:'grid', gridTemplateColumns:'320px 1fr', minHeight:'calc(100vh - 56px)' }}>
          ...
          <div>
            {chats.map(c => (
              <button key={c.wa} onClick={()=>setActiveWa(c.wa)}
                ...
                style={{ width:'100%', textAlign:'left', padding:'12px 12px', color:'#fff', background: activeWa===c.wa ? 'rgba(255,255,255,.1)' : 'transparent', border:'0', cursor:'pointer' }}>
        ...
        <section style={{ display:'flex', flexDirection:'column' }}>
          ...
          <div ref={scrollRef} style={{ flex:1, overflow:'auto', padding:16, background: palette.negro }}>
            {messages.map(m => (
              <div key={m._key}
                   style={{ maxWidth:'70%', marginBottom:8, padding:'12px', borderRadius:16, background: m.direction==='in' ? '#1f2937' : palette.rojo, color:'#fff', marginLeft: m.direction==='in' ? 0 : 'auto', marginRight: m.direction==='in' ? 'auto' : 0 }}>
          ...
          <div style={{ padding:'10px 16px', borderTop:'1px solid #1f1f22', background: palette.gris, color:'rgba(255,255,255,.8)', fontSize:12 }}>
            • Mensajes ordenados por timestamp y desduplicados por <code>id</code>. Si ves alternancia, verificá que el webhook guarde <code>id</code> único por mensaje.
          </div>
        </section>
      </main>
    </div>
